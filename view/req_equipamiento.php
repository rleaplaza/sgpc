		    ...
		    try{
			 ...
			 if($consulta->rowCount()>0){
			  ...
			  $idopcion=$_GET["idopcion"];
			   ...
			   ?>
               <img src="../images/requerimiento.jpg" height="58" width="58"><br>
               <h2><legend>Seleccione el proyecto para enviar al formulario de requerimiento</legend></h2><br>
               <form method="post">
               <table>
               ...
               <tr><td><select name="proveedor" id="proveedor">
               <?php 
			   $query=$dbh->prepare("select *from proveedor");
			   $query->execute();
			   if($query->rowCount()>0){
				   foreach($query->fetchAll() as $fila){
					   ?>
                       <option value="<?php echo $fila["IDproveedor"];?>"><?php echo $fila["empProveedora"];?>
                       <?php
					   }
				   }
			   ?>
               ...
               <tr><td><input type="submit" id="sub" value="Nuevo Requerimiento" onclick="this.form.action='formReq_equipamiento.php'">
                </td>
               </tr>
               </table>
               </form>
