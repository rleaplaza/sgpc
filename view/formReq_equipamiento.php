...
<script type="text/javascript">
	window.addEvent('domready',function(){
		...
		ajaxFace = new LightFace.Request({
					url: 'registros/addReq_equipamiento.php',
					buttons: [
					...
					request: { 
						data: { 
							...
							IDequipamiento:document.id('idequipamiento').value || 'ID no encontrado',
...
Para enviar la solicitud por email, presionar la imagen PDF, adjuntarlo en el campo archivo y presionar el botón Enviar solicitud">
    ...
    <fieldset>
    <legend>Formulario de requerimiento de equipamiento</legend> 
    ...
    <tr><td><label>Equipamiento</label></td><td><select name="equipamiento" class="equipamiento" id="idequipamiento">
    <?php
	$proveedor=$_POST["proveedor"];
    $sql=$dbh->prepare("select *from maquinaria where IDproveedor=? order by descripcion asc");
	$sql->bindParam(1,$proveedor);
	$sql->execute();
	if($sql->rowCount()>0){
		foreach($sql->fetchAll() as $fila){
		?>
        <option value="<?php echo $fila["IDmaquinaria"]?>"><?php echo $fila["descripcion"];?>
        <?php
		}
	}
	?>
    </select></td></tr>
    <tr><td><label>Cantidad a solicitar</label></td><td><input type="text" name="cantidad" id="cantidad" class="cantidad" maxlength="3" required><img src="../images/ayuda.jpg" height="20" width="20" title="Escriba un valor numérico entero"></td></tr>
