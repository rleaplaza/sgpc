	...
	window.addEvent('domready',function(){
			...
			document.id('sub').addEvent('click',function(){
				...
				ajaxFace = new LightFace.Request({
					url: 'registros/addItem1.php',
					buttons: [
						{ title: 'Cerrar', event: function() { this.close(); },color:'blue' }
					],
					request: { 
						data: { 
							IDmaquinaria: document.id('idmaquinaria').value || 'ID no capturado',
...
<img src="../images/ayuda.jpg" height="30" width="30" title="Llenar el formulario con la correspodiente información de la subfase y la descripción"/><br>
  ...
  ?>
<fieldset>
     <legend>Formulario de registro de items de maquinaria</legend>
     <form method="post" class="usuario" id="formulario" name="form" >
     <table>
     <div><tr><td><label>Descripción de maquinaria</label></td><td><input type="text" name="desc" class="desc" disabled="disabled" value="<?php echo $res["descripcion"];?>"></td>
       <input type="hidden" value="<?php echo $idmaquinaria;?>" name="idmaquinaria" class="maquinaria" id="idmaquinaria"></tr></div>
