...
<body>
<input type="submit" value='Volver a listado de proveedores' onClick='history.back();' id="sub">
<?php
if(isset($_SESSION["username"])){
	require_once("../db/connect.php");
	if(isset($_GET["iduser"])){
	$iduser=$_GET["iduser"];
	$sql=$dbh->prepare("SELECT usuario.USR_UID, proveedor.CI
                        FROM usuario, proveedor
                        WHERE usuario.USR_UID=proveedor.USR_UID
						and usuario.USR_UID = ?");
    ...
    <table align="left">
    <div><tr><td><label>User ID:</label></td><td><input type="text" id="userid" class="userid" name="userid" disabled="disabled" value="<?php echo $iduser;?>"/></td></tr></div>
    <div><tr><td><label>Nombres de Proveedor:</label></td><td><input type="text" id="nombre" class="nombre" name="nombre" placeholder="Ronald" maxlength="40" /></td></tr></div>
    <div><tr><td><label>Apellido parterno:</label></td><td> <input type="text" id="app" class="app" name="app" placeholder="Loza" maxlength="40" /></td></tr></div>
     <div><tr><td><label>Apellido marterno:</label></td><td><input type="text" id="apm" class="apm" name="apm" placeholder="Perez" maxlength="40" /> </td></tr></div>
      ...
      <div><tr><td><label>Empresa proveedora:</label></td><td><input type="text" id="empresa" class="empresa" name="empresa" placeholder="Empresa de servicio de maquinaria y equipos" maxlength="40" /> </td></tr></div>
      <div><tr><td><label>Dirección de la empresa:</label></td><td><input type="text" id="dir" class="dir" name="dir" placeholder="av 6 de marzo El Alto" maxlength="80" /> </td></tr></div>
  ...
  <tr><td><button  class="boton_envio">REGISTRAR</button></td><td><input class="reset" type="reset" value="VACIAR FORMULARIO"></td></tr>
    ...
    <script type="text/javascript" src="../js/nuevoProveedor.js"></script>
    <?php
