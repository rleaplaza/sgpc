...
<legend><h2>Información de la profesión a editar</h2></legend>
<?php
if(isset($_SESSION["username"])){
if(isset($_GET["idprofesion"])){
	try{ require_once("../../db/connect.php");
	     $sql=$dbh->prepare("select *from profesion where IDprofesion=?");
		 $sql->bindParam(1,$_GET["idprofesion"]);
		 $sql->execute();
		 if($sql->rowCount()>0){
        ...
        <table>
        <div><tr><td><label>Profesión</label></td>
                 <td><input type="text" id="profesion" name="profesion" class="profesion" value="<?php echo $row["nombre"];?>" maxlength="10" readonly></td></tr></div>
        <div><tr><td><label>Descripcion</label></td>
         ...
         <div><tr><td><label>Hora de creación</label></td>
                 ...
                 <input type="hidden" name="iddprofesion" class="idprofesion" value="<?php echo $row["IDprofesion"];?>">
          <div class="ultimo">
     ...
     <script type="text/javascript" src="../../js/edit/editProfesion.js"></script>
                 <?php
				 }
			 }
