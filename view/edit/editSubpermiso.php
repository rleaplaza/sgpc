...
<head>
<meta charset="utf-8">
<title>Edicion de subpermisos</title>
...
if(isset($_GET["idpagina"])){
	try{ require_once("../../db/connect.php");
	     $sql=$dbh->prepare("select *from pagina_opcion where IDpagina=?");
		 $sql->bindParam(1,$_GET["idpagina"]);
		 $sql->execute();
		 if($sql->rowCount()>0){
        ...
        <table>
        <div><tr><td><label>Codigo de pagina</label></td>
                 <td><input type="text" value="<?php echo $row["IDpagina"]?>" name="idpagina" class="idpagina "disabled="disabled"></td></tr></div>
        <div><tr><td><label>Nombre de pagina</label></td>
                 <td><input type="text" name="subpermiso" class="subpermiso" value="<?php echo $row["nombre"];?>" maxlength="30"><img src="../../images/editable.jpg" height="23" width="23" title="campo editable"></td></tr></div>
     ...
     <script type="text/javascript" src="../../js/edit/editSubpermiso.js"></script>
                 <?php
				 }
			 }
