...
function Header()
{
	...
	$this->Text(20,13,'Requerimiento de equipamiento',0,'C', 0);
	$this->Ln(30);
...
function Footer()
{
	...
	$this->Cell(100,10,'Requerimiento de equipamiento',0,0,'L');
	...
	$pdf->Cell(90);
	$pdf->Cell(19,20,'Requerimiento de equipamiento');
    ...
    $pdf->SetTextColor(255);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('NRO','DESCRIPCION', 'CANTIDAD'));
			}	
	$sql=$dbh->prepare("SELECT m.descripcion, dr.cantidad
                        FROM maquinaria AS m, det_req_equipamiento AS dr, req_equipamiento AS req
                        WHERE m.IDmaquinaria = dr.IDmaquinaria
...
$pdf->Output('archivos/requerimientoMaquinaria.pdf','D');
?>