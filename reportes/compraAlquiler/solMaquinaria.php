...
function Footer()
{
	...
	$pdf->Cell(90);
	$pdf->Cell(19,20,'Solicitud de equipamiento');
    ...
    $pdf->SetWidths(array(40, 70, 30, 50,40));
    ...
    $pdf->SetTextColor(255);
		...
		for($i=0;$i<1;$i++)
			{
				$pdf->Row(array('NRO DE SOLICITUD','DESCRIPCION', 'CANTIDAD', 'UNIDAD','FECHA'));
			}	
	$sql=$dbh->prepare("SELECT s.nro_solicitud, p.nombre, m.descripcion, s.cantidad_sol, s.estado,                        fechaSolicitud
                       FROM solicitud_maquinaria AS s, maquinaria AS m, proyecto AS p
	...
	for ($i=0; $i<$numfilas; $i++)
		{
			...
			if($i%2 == 1)
			{
    			...
    			$pdf->SetTextColor(0);
				$pdf->Row(array($row[0], $row[2], $row[3], $row[4],$row[5]));
			}
			else
			{
...
$pdf->Output('archivos/solicitudMaquinaria.pdf','D');
?>